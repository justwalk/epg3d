...
<!-- 查询咨询下新闻的前24条数据 -->
<epg:query queryName="getSeverialItems" maxRows="24" var="newsItems" pageBeanVar="pageBean" pageIndexParamVar="pageIndex" pageTurnIsLoop="true">
    <epg:param name="categoryCode" value="${infomationItems[0].itemCode}" type="java.lang.String"/>
</epg:query>

<!-- 查询咨询下的专题的前4条数据 -->
<epg:query queryName="getSeverialItemsIncludePic" maxRows="4" var="subjectItems">
...
{		
	...
	if(leaveFocusId!=""&&document.getElementById(leaveFocusId+"_a")){
		...
		document.getElementById("main1_a").focus();
	}
}
...
<!-- 背景图片 -->
<epg:img defaultSrc="./images/infomationNewsImg.jpg" src="../${templateParams['bgImg']}" id="infomationBgImg" left="0" top="0" width="1280" height="720"/>
<!-- 二级菜单选中图标 -->
<epg:img src="./images/infomation1.png" id="newsImg" left="347" top="134" width="149" height="29"/>
...
<epg:img src="./images/dot.gif" href="javascript:pageUp();"  id="area_upPage"
         left="79" top="175" width="85" height="29" pageop="up" keyop="pageup"
...
<epg:img src="./images/dot.gif" href="javascript:pageDown();" id="area_downPage" 
		 left="179" top="175" width="85" height="29" pageop="down" keyop="pagedown"
...
<!-- 文字列表 -->
<epg:set var="crosswiseSize" value="12"/>
<epg:set var="left" value="80"/>
<epg:set var="top" value="215"/>
<epg:set var="top" value="215"/>
<epg:set var="hspacing" value="0"/>
<epg:set var="vspacing" value="0"/>
<epg:forEach items="${newsItems}" var="newsItem" varStatus="idx">
	<epg:navUrl obj="${newsItem}" indexUrlVar="indexUrl"/>
	<epg:choose>
		<epg:when test="${idx.count == 1}">
	...
	<epg:text left="${left+hspacing}" top="${top+vspacing}" width="360" height="30" fontFamily="黑体" fontSize="22"
			  color="#ffffff" chineseCharNumber="15" dotdotdot="…" text="${newsItem.title}" align="left"/>
	<epg:img src="./images/dot.gif" href="${indexUrl}" id="main${idx.count}" rememberFocus="true"
			 defaultfocus="${isDefaultfocus}" left="${left+hspacing-5}" top="${top+vspacing-4}" width="363" height="32" 
	         onfocus="itemOnFocus('main${idx.count}_img','infomationHight.png');" 
	         onblur="itemOnBlur('main${idx.count}_img','dot.gif');"/>
	<epg:choose>
		<epg:when test="${idx.index < crosswiseSize}">
			<epg:choose>
				<epg:when test="${(idx.index+1) % 3 > 0}">
					<epg:set var="hspacing" value="${hspacing+385}"/>
				</epg:when>
				<epg:when test="${(idx.index+1) % 3 == 0}">
					<epg:set var="hspacing" value="0"/>
					<epg:set var="vspacing" value="${vspacing+44}"/>
				</epg:when>
			</epg:choose>
		</epg:when>
		<epg:otherwise>
			<epg:choose>
				<epg:when test="${(idx.index+1) % 2 > 0}">
					<epg:set var="hspacing" value="${hspacing+385}"/>
				</epg:when>
				<epg:when test="${(idx.index+1) % 2 == 0}">
					<epg:set var="hspacing" value="0"/>
...
<!-- 图片列表 -->
<epg:grid items="${subjectItems}" var="subjectItem" column="1" row="4" left="840" top="380" height="260" width="365" posVar="pos" indexVar="idx" vcellspacing="10" >
	<epg:navUrl obj="${subjectItem}" indexUrlVar="indexUrl"/>
	<epg:img src="../${subjectItem.itemIcon}" left="${pos[idx-1].x}"  top="${pos[idx-1].y}" width="363" height="59"/>
	<epg:img src="./images/dot.gif" href="${indexUrl}&pi=${idx}" id="subject${idx}" rememberFocus="true"
			 left="${pos[idx-1].x}"  top="${pos[idx-1].y}" width="365" height="60" 
	         onfocus="itemOnFocus('subject${idx}_img','infomationSubjectFocus.png');"  onblur="itemOnFocus('subject${idx}_img','dot.gif');"/>
</epg:grid>
