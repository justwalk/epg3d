...
<%
	//即将上线、下线展示
	Date now=new Date();
	Date nextWeek=new Date(now.getTime()+1000*3600*24*7);
...
<div id="leftDiv">
<epg:img src="./images/sd_theaterList_jk.jpg" width="640" height="720"/>
...
<epg:grid left="25" top="202" width="590" height="420" row="2" column="8" items="${contentCategoryItems}"
	...
	<epg:choose>
		<epg:when test="${contentCategoryItem.validTime > lastWeek}">
			<epg:img id="tag${curIdx}" src="./images/new.png" width="26" height="52" left="${positions[curIdx-1].x}" top="${positions[curIdx-1].y}" />
		</epg:when>
		<epg:when test="${contentCategoryItem.expireTime < nextWeek}">
			<epg:img id="tag${curIdx}" src="./images/last.png" width="26" height="52" left="${positions[curIdx-1].x}" top="${positions[curIdx-1].y}" />
		</epg:when>
	</epg:choose>
</epg:grid>
</div>
<!-- ********************************************************* -->
<div id="r_leftDiv">
<epg:img src="./images/sd_theaterList_jk.jpg" width="640" height="720" left="640"/>
...
<epg:grid left="675" top="202" width="590" height="420" row="2" column="8" items="${contentCategoryItems}"
	...
	<epg:choose>
		<epg:when test="${contentCategoryItem.validTime > lastWeek}">
			<epg:img id="r_tag${curIdx}" src="./images/new.png" width="26" height="52" left="${positions[curIdx-1].x}" top="${positions[curIdx-1].y}" />
		</epg:when>
		<epg:when test="${contentCategoryItem.expireTime < nextWeek}">
			<epg:img id="r_tag${curIdx}" src="./images/last.png" width="26" height="52" left="${positions[curIdx-1].x}" top="${positions[curIdx-1].y}" />
		</epg:when>
	</epg:choose>
</epg:grid>
</div>
